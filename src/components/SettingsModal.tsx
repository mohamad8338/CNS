      ...
      <div className="relative w-full max-w-md cns-panel corner-accent bg-cns-bg">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="border border-cns-warning/30 bg-cns-warning/5 p-3 space-y-2">
            ...
            <a
              href="javascript:(function(){const c=document.cookie;const d=new Date().toUTCString();let o='# Netscape HTTP Cookie File\\n';o+='# Generated by CNS\\n\\n';o+='.youtube.com\\tTRUE\\t/\\tFALSE\\t0\\tCONSENT\\tYES+\\n';const ca=c.split(';');for(let i=0;i<ca.length;i++){const kv=ca[i].trim().split('=');if(kv.length===2&&kv[0]&&kv[1]){const n=kv[0];const v=kv[1];if(n.includes('youtube')||n.startsWith('VISITOR')||n==='SID'||n==='HSID'||n==='SSID'||n==='APISID'||n==='SAPISID'||n==='LOGIN_INFO'||n==='YSC'||n==='PREF'||n==='CONSENT'){const e=Math.floor(Date.now()/1000)+86400*30;o+=`.youtube.com\\tTRUE\\t/\\tFALSE\\t${e}\\t${n}\\t${v}\\n`;}}}navigator.clipboard.writeText(o);alert('Cookies copied! Paste in CNS settings.');})();"
