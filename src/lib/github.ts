  ...
  download:
    ...
    steps:
      ...
      - name: Split large files
        run: |
          # Split files >100MB into 50MB chunks
          ...
          for file in *; do
            if [ -f "\$file" ]; then
              ...
              if [ "\$size" -gt "\$MAX_SIZE" ]; then
                ...
                base="\${file%.*}"
                ext="\${file##*.}"
                split -b "\$CHUNK_SIZE" "\$file" "\${base}.part"
                ...
                # Update metadata
                if [ -f "\${base}.json" ]; then
                  python3 -c "import json,sys; f=open(sys.argv[1],'r'); d=json.load(f); f.close(); d['split']=True; d['parts']=int(sys.argv[2]); d['original_size']=int(sys.argv[3]); d['ext']=sys.argv[4]; f=open(sys.argv[1],'w'); json.dump(d,f,indent=2); f.close()" "\${base}.json" "\$part_count" "\$size" "\$ext"
                fi
