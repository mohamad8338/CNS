...
این دستور تمام بخش‌های تقسیم‌شده را ترکیب کرده و یک فایل کامل `complete.zip` می‌سازد.

### ۳. اکسترکت کردن
- فایل `complete.zip` را اکسترکت کنید
- فایل اصلی ویدیو دریافت می‌شود
...
**مراحل:**
۱. همه فایل‌ها را در یک پوشه دانلود کنید
۲. در ترمینال به آن پوشه بروید و اجرا کنید:
```bash
zip -s 0 video.zip --out complete.zip
```
۳. فایل `complete.zip` را اکسترکت کنید → `video.mp4`
